...
class ScanHardwareSource(HardwareSource.HardwareSource):
    ...
    def __init__(self, stem_controller_: stem_controller.STEMController, device, hardware_source_id: str, display_name: str):
        ...
        _channel_count = self.__device.channel_count
        if (hasattr(self.__device, "all_channel_count")):
            _channel_count = self.__device.all_channel_count
            channel_info_list = [
                ChannelInfo("abcdefghijklmnopqrstuv"[channel_index], self.__device.get_all_channel_name(channel_index)) for
                channel_index in range(_channel_count)]
        else:
            channel_info_list = [ChannelInfo(self.__make_channel_id(channel_index), self.__device.get_channel_name(channel_index)) for channel_index in range(_channel_count)]
    ...
    def __get_initial_profiles(self) -> typing.List[typing.Any]:
        if hasattr(self.__device, "get_initial_profiles"):
            profiles = self.__device.get_initial_profiles()
        else:
            profiles = list()
    ...
    @property
    def all_channel_count(self) -> int:
        if hasattr(self.__device, "all_channel_count"):
            count = self.__device.all_channel_count
        else:
            count = self.__device.channel_count
        return count

    @property
    def channel_count(self) -> int:
        return self.__device.channel_count
    ...
    def get_channel_index_for_data_channel_index(self, data_channel_index: int) -> int:
        return data_channel_index % self.all_channel_count
    ...
    def set_selected_profile_index(self, profile_index):
        # orsay device requires to know the current profile index
        changed = self.__current_profile_index != profile_index
        self.__current_profile_index = profile_index
        if changed:
            if self.__device.scan_device_id == "orsay_scan_device":
                self.__device.profile_index = profile_index
                self.__channel_states_changed(
                    [self.get_channel_state(i_channel_index) for i_channel_index in range(self.channel_count)])
    ...
    def __make_channel_id(self, channel_index) -> str:
        if self.__device.scan_device_id == "orsay_scan_device":
            select1 = "abcdefghij"
            select2 = "klmnopqrst"
            if self.__current_profile_index == 0:
                select1 = "abcd"
                select2 = "klmn"
            if self.__current_profile_index == 1:
                select1 = "abef"
                select2 = "klop"
            if self.__current_profile_index == 2:
                select1 = "abgh"
                select2 = "klqr"
            select = select1 + select2
            return select[channel_index]
        else:
