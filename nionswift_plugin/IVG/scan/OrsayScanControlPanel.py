...
class ScanControlStateController:
    ...
    def __init__(self, scan_hardware_source: scan_base.ScanHardwareSource, queue_task, document_model, channel_id: typing.Optional[str]):
        self.profiles = {_("Puma"): 0, _("Rabbit"): 1, _("Frame"): 2}
        self.__scan_hardware_source = scan_hardware_source
        #Marcel
        # add capability to get profiles from device
        self.__device = None
        # will succeed with orsayscan
        self.__device = scan_hardware_source.scan_device
        if hasattr(self.__device, "get_initial_profiles"):
            profile_items = self.__device.get_initial_profiles()
            if "name" in profile_items[0]:
                self.profiles.clear()
                index = 0
                for nm in profile_items:
                    self.profiles[nm["name"]]=index
                    index = index + 1
        ...
        self.on_channel_state_changed : typing.Optional[typing.Callable[[int, str, str, bool, bool], None]] = None
    ...
    def initialize_state(self):
        ...
        if self.on_profiles_changed:
            ...
            profile_items = list(self.profiles.items())
    ...
    def handle_change_profile(self, profile_label):
        ...
        if profile_label in self.profiles:
            #self.__scan_hardware_source.set_selected_profile_index(ScanControlStateController.profiles[profile_label])
            self.__scan_hardware_source.set_selected_profile_index(self.profiles[profile_label])
            self.__update_channels()
    ...
    def __channel_state_changed(self, channel_index: int, channel_id: str, name: str, enabled: bool) -> None:
        if self.on_channel_state_changed:
            ...
            self.on_channel_state_changed(channel_index, channel_id, name, enabled, is_subscan_channel)
...
class ScanControlWidget(Widgets.CompositeWidgetBase):
    ...
    def __init__(self, document_controller: "DocumentController.DocumentController", scan_controller: scan_base.ScanHardwareSource):
        ...
        def channel_state_changed(channel_index: int, channel_id: str, name: str, enabled: bool, is_subscan_channel: bool) -> None:
        ...
        async def update_subscan_state(subscan_state: stem_controller.SubscanState, line_scan_state: stem_controller.LineScanState) -> None:
            ...
            for channel_index, (channel_id, name, channel_enabled) in self.__channel_states.items():
                ...
                channel_state_changed(channel_index, channel_id, name, self.__state_controller.get_channel_enabled(channel_index),
                                  is_subscan_channel)
        ...
        if hardware_source.features.get("is_scanning", False) and hardware_source.hardware_source_id == "orsay_scan_device":
            ...
            class ScanDisplayPanelControllerFactory:
                ...
                def build_menu(self, display_type_menu, selected_display_panel):
                    ...
                    actions = list()
                    # orsay does have specific data_channel for each profile
                    ...
                    for channel_index in range(hardware_source.channel_count):
                        ...
                        # orsay_scan does not always has all its channels in a profile, so some channels may not be valid here
                        # just ignore them
